...
public class MessageShowServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int connte=dao.sun(user.getUser_id());
		request.setAttribute("connte", connte);
		List<Message> messages=dao.selectAll(user.getUser_id(),(pagevalue-1)*5,5);
		request.setAttribute("messages", messages);
		request.getRequestDispatcher("Message/MessageShow.jsp").forward(request, response);
	}
